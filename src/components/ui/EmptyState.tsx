...
export function EmptyState({ icon: Icon, title, description, action }: EmptyStateProps) {
  return (
    <Card blur className="p-12 text-center">
      <motion.div variants={animations.container} initial="hidden" animate="visible">
        ...
        <motion.h3 variants={animations.item} className="text-2xl font-bold text-gray-900 mb-2">
          {title}
        </motion.h3>
        <motion.p variants={animations.item} className="text-gray-600 mb-8 max-w-md mx-auto">
          {description}
        </motion.p>
        {action && <motion.div variants={animations.item}>{action}</motion.div>}
      </motion.div>
    </Card>
  );
}
